describe('layer-parser', function() {
    ...
    describe('arguments', function () {
        ...
        it('should detect activation argument type', function() {
            let activation = dense.arguments.find(arg => arg.name === 'activation');
            assert.equal(activation.type, 'activation');
        });

        ['regularizer', 'constraint', 'initializer'].forEach(type => {
            it(`should detect ${type} argument type`, function() {
                let args = dense.arguments.filter(arg => arg.name.includes(type));
                let types = args.map(arg => arg.type);
                assert.equal(args[0].type, type);
                assert(types.every(t => t === type), `expected regularizer types but got ${types}`);
            });
        });
